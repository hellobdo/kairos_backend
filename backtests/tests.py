...
class BacktestCalculationsTest(unittest.TestCase):
    ...
    def test_exits_respect_config(self):
        """Test that exits respect the configured risk/reward ratios."""
        # Get exit configuration
        exit_config = self.backtest.get_exits_config(self.backtest.EXIT_CONFIG_ID)
        if not exit_config:
            self.fail("Failed to get exit configuration")
            
        # Currently only supporting fixed exits
        if exit_config['type'] != 'fixed':
            self.skipTest("Currently only testing fixed exits")
        ...
        for _, trade in self.valid_trades.iterrows():
            ...
            # For winning trades, check if R:R is at least the target
            # For losing trades, check if R:R is less than target (stopped out)
            is_winner = trade['PnL'] > 0
            
            # Update statistics
            if is_winner:
                total_winners += 1
                winners_rr_sum += actual_rr
            else:
                total_losers += 1
                losers_rr_sum += actual_rr
            
            if is_winner and actual_rr < target_rr:  # Allow 5% tolerance for winning trades
                failures_winners.append({
                    'Entry Date': trade['Entry Date'],
                    'Exit Date': trade['Exit Date'],
                    'Entry Price': entry_price,
                    ...
                    'PnL': trade['PnL'],
                    'Issue': 'Winner below target R:R'
                })
            elif not is_winner and actual_rr >= target_rr:
                failures_losers.append({
                    'Entry Date': trade['Entry Date'],
                    'Exit Date': trade['Exit Date'],
                    'Entry Price': entry_price,
                    'Exit Price': exit_price,
                    ...
                    'Actual R:R': actual_rr,
                    'PnL': trade['PnL'],
                    'Issue': 'Loser above target R:R'
                })
        
        # Print overall statistics
        print(f"\nExit Configuration Analysis:")
        print(f"Target R:R: {target_rr}")
        print(f"\nOverall Trade Statistics:")
        print(f"Total Trades: {len(self.valid_trades)}")
        print(f"Total Winners: {total_winners} ({(total_winners/len(self.valid_trades))*100:.1f}%)")
        print(f"Total Losers: {total_losers} ({(total_losers/len(self.valid_trades))*100:.1f}%)")
        
        if total_winners > 0:
            avg_winner_rr = winners_rr_sum / total_winners
            print(f"\nWinners Analysis:")
            print(f"Average R:R for Winners: {avg_winner_rr:.2f}")
            print(f"Winners Below Target: {len(failures_winners)} ({(len(failures_winners)/total_winners)*100:.1f}%)")
        
        if total_losers > 0:
            avg_loser_rr = losers_rr_sum / total_losers
            print(f"\nLosers Analysis:")
            print(f"Average R:R for Losers: {avg_loser_rr:.2f}")
            print(f"Losers Above Target: {len(failures_losers)} ({(len(failures_losers)/total_losers)*100:.1f}%)")
        
        # Print failure details
        if failures_winners:
            print(f"\nWinner Failures (first 5):")
            for failure in failures_winners[:5]:
                print(f"\nTrade from {failure['Entry Date']} to {failure['Exit Date']}:")
                print(f"  Entry: ${failure['Entry Price']:.2f}")
                print(f"  Exit: ${failure['Exit Price']:.2f}")
                print(f"  Stop: ${failure['Stop Price']:.2f}")
                print(f"  Target R:R: {failure['Target R:R']:.2f}")
                print(f"  Actual R:R: {failure['Actual R:R']:.2f}")
                print(f"  PnL: ${failure['PnL']:.2f}")
        
        if failures_losers:
            print(f"\nLoser Failures (first 5):")
            for failure in failures_losers[:5]:
                print(f"\nTrade from {failure['Entry Date']} to {failure['Exit Date']}:")
                print(f"  Entry: ${failure['Entry Price']:.2f}")
                print(f"  Exit: ${failure['Exit Price']:.2f}")
                print(f"  Stop: ${failure['Stop Price']:.2f}")
        ...
        total_failures = len(failures_winners) + len(failures_losers)
        self.assertEqual(total_failures, 0, 
                        f"Found {total_failures} trades with incorrect exits based on R:R targets "
                        f"({len(failures_winners)} winners, {len(failures_losers)} losers)")
